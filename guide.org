...
# CSS for HTML export styling
#+html_head: <style> .theorem { margin: 1em 0; padding: 1em; background: #f4f4f4; border-left: 4px solid #333; } </style>
#+html_head: <style> .theorem-title { font-weight: bold; margin-bottom: 0.5em; display: block; } </style>
#+html_head: <style> .definition { margin: 1em 0; padding: 1em; background: #f4f4f4; border-left: 4px solid #333; } </style>
#+html_head: <style> .definition-title { font-weight: bold; margin-bottom: 0.5em; display: block; } </style>
#+language: en
#+select_tags: export
...
** What I am assuming you know or will learn on your own.
   Take a look at [[https://github.com/brittAnderson/uwlooP390]] and [[https://brittanderson.github.io/uwlooP390/]]
...
1. For which of our examples is the Rescorla-Wagner Model a good fit?
2. RW belongs to which of Marr's levels?
3. What is the level that we should be working at?
...
*** Are they a good model system for the mind? That is, does an LLM think like you do?
...
* Where will your grade come from?
...
** Why incentivize classroom time?
...
** Why is "doing" so important for this course?
...
** I find this stuff hard and am worrying about failing
...
* Day 1 ended here :noexport:
* Computational Neuroscience 
** Slime assignment 
*** Slime                                                          :noexport:
[[file:f2026/misc/slime-mold/slime-mold_test.pdf]]

** What are examples of research combining cognitive, computational, and neuroscience methods?
  1. Form groups of two (or three if odd number) (even numbers allowed llm use;odd numbers any resources, including on line, but *not* llm).
  2. Go to the following web site: [[https://2026.ccneuro.org/poster-sessions/?view=all][2026 Posters Cognitive Computational Neuroscience meeting]].
     1. Pick a poster.
	Note it's label (letter-number; on the left)
     2. Be ready with a two sentence summary.
     3. Tell us why it is a good example of computational modeling of either mind or brain.
     4. Why you chose it.
*** Evaluating Academic Work :exercise:
**** Exercise :noexport:
   Each group offers an explanation *without* llm use for : [[https://2026.ccneuro.org/poster/?id=aX1J1ByZCF][Poster to Explain]]
...
#+caption: The original uploader was Icez at English Wikipedia.
[[file:./images/Forgetting_curve_decline.svg.png]]

#+begin_src haskell :session :cache no :results silent :exports none
:set -i(locate-dominating-file-equivalent)
#+end_src

*** A Mathematical Function
#+begin_src haskell :session :cache no :results file graphics :file ./images/expdecay.png :exports both
...
**** A digression on code
     The above example is coded in [[https://www.haskell.org/][Haskell]]. 
     a) What makes Haskell different from other programming languages?
     b) When might it be a good/bad choice?
     c) Do you need to learn to write code?
	Why or why not?
     d) Regardless of your answer, how do we get LLMs to support us? Or should we not use LLMs at all?

**** Programming Exponential Decay :exercise:
...
**** Same Exercise as Homework with one of these languages /that I will assign/. For this exercise you are supposed to use an LLM. See the Learn Dropbox for additional instructions. 
     1) Racket (common lisp is okay too, but may be harder to install)
     2) Go
     3) Rust
     4) Lean4
     5) Typescript
     6) Ruby
     7) Ocaml
     8) APL (or any variant of APL you can get to work)
     9) Prolog
...
*** More on the fitting curves to Ebbinghaus Data
    What can we infer from the similarity between Figure [[forgetting-curve]] and Figure [[exponential-function]] ?

    What equations did Ebbinghaus fit to his own data? And what have other suggested?

    You can find the answers here: [[https://doi.org/10.1371/journal.pone.0120644][Replication and Analysis of Ebbinghaus' Forgetting Curve]].
    ...
    How might we reconcile the exponential and power function versions? An answer might be [[https://doi.org/10.3758/s13423-011-0076-y][here]].

    Once again, what is the difference between curve fitting and a mathematical (or computational) model?
    
** Reprise
   1. What does COMPUTATIONAL cognitive neuroscience/science/psychology mean?
      1. Consider checking out [[https://www.sciencedirect.com/science/article/pii/S0960982219306141?via%3Dihub][Bain, What is Cognition?]]
   2. What are our first impressions of what research in the field looks like?
   3. What are our early impressions about the use of LLM for
      - answering conceptual questions?
      - coding implementations?

* Neurally Inspired Models 1: The cellular level
  
  What is the mathematical tool?

  Differential Equations.

  But first, let's understand the phenomena where we intend to use them.
  
  Our Process:
  1. Before modeling, understand what we are trying to model.
     In this case, the action potential.
  2. Review prior work.
     Let's discuss what this means.
  3. How do we hope to benefit from using a computational model?
  4. The above should drive our focus to particular mathematical tools.
     I will argue that we should be led to use differential equations here. You should be prepared to tell me why that is the right choice (or not; as long as you have principled arguments).

     For this example we need to extend the process just described.

     Understand what we are modeling and what we are using to model with.
     1. Simple electronics.
     2. Simple differential equations.
  5. Implement simple computational versions.
     Make a reference implementation. That is make it good, before you make it fast.

** That which you cannot code, you do not understand.
   My rephrasing of Richard Feynman's "I don't understand anything I can't build."
...
** What is an action potential?
   ...
   5. Who were [[https://en.wikipedia.org/wiki/A_Logical_Calculus_of_the_Ideas_Immanent_in_Nervous_Activity][McCulloch and Pitts]] and why are they relevant?
      [[https://link.springer.com/article/10.1007/BF02478259][A logical calculus]]
   ...
   9. What math is important for modeling spiking neurons?
** Notation
...
*** Notation Scavenger Hunt :exercise:
...
**** Basic Numbers (solutions) :noexport:
     
** Differential Equations :exercise:
   We want to discuss these questions. I will give you a few minutes. You can either work alone. Talk to a neighbor. Use an LLM (if you do, note which one and your prompts). To get us rolling try to come up with your best succinct answer to the first two questions. I will call on several of you, and we will vote on the best one. 
   1. What is a slope of a line?
   2. What is the slope of a curve?
   3. What is a derivative?
   4. What is the *definition* of a derivative?
      $\frac{dx}{dy} = \frac{f(x + \Delta x) - f(x)}{x+\Delta x - x}$
   5. What is a differential equation (aka de)?
...
#+begin_src haskell :session  :results file graphics :file ./images/derivExample.png :exports both :cache no
...
Now let's test it:
#+begin_src haskell :session  :cache no :results output :exports both
  :! pwd
  :load "./src/Sqrt.hs"
...
#+RESULTS:
: /home/britt/gitRepos/compNeuroIntro420
: [1 of 1] Compiling Sqrt             ( src/Sqrt.hs, interpreted )
: Ok, one module loaded.
: 3.0000005410641766
: 2.000004911675504
: 5.204827869200383

*** Your turn :assignment:in-class:
    Working on your own on in any group $\le$ 3 members produce code in any other language than Haskell to compute the fourth root (quartic). You are to implement the method I demonstrate to compute it, not simply to call a pre-built function that does it. 


    
** Using DEs - Practicing with Springs :homework:

Our Hodgkin-Huxley model will be will require us to use several different equations. And though the integrate and fire neuron only requires one, a neuron is still rather abstract as a computational entity. Therefore, we will begin our DE coding practice with a simple, idealized spring. 
...
#+begin_src haskell :session  :results file graphics :file ./images/spring.png :exports both :cache no
  :load "./src/Spring.hs"
...
*** Try with a *Damped Oscillator*                                 :homework:
...
***  Classroom follow-up                                           :noexport:
Adapt your code to run in a browser applet that allows users to change the constants themselves and see the effects on the curves. See how far you can get on this by vibe coding. Check the code the the model wrote and see if you can understand how it compares to yours. We will want to review models and prompts.

*** What can you do with the University's Copilot                  :noexport:
lookup in copilot [[file:src/copilot/][dir]]
[[https://copilot.cloud.microsoft/chat/conversation/4227ee5d-8a94-40d5-9c13-67a8c45c4d08?fromcode=cmmm35wux52&lhredirected=1&es=AutoSignIn&redirfrom=cosmicRingCookie][oscillator-chat-copilot]]
...
#+begin_src haskell :session  :results file graphics :file ./images/iandf.png :exports both :cache yes
  :load ./src/IandF.hs
...
#+RESULTS[dd5377738ec68af521597a2074a806e67bf498bf]:
...
#+begin_src haskell :session  :results file graphics :file ./images/handh.png :exports both :cache yes
  :load ./src/HandH.hs
...
#+RESULTS[45cce560bd120a655b9c792ab5fb8b29af0ff7f3]:
...
#+begin_src haskell :session  :exports both :cache no :results output :exports none
  :load ./src/BusyBeaver.hs
#+end_src

#+RESULTS:
: [1 of 1] Compiling BusyBeaver       ( src/BusyBeaver.hs, interpreted )
: Ok, one module loaded.

#+begin_src haskell :session  :exports both :cache no :results output
  tape $ iterate updTM initialTM  !! 7
#+end_src
...
#+BEGIN_SRC haskell :session  :exports both :results output :cache no
  :set -w
...
#+begin_src haskell :session  :results file graphics :file ./images/rule30.png :exports  both :cache no
  :load ./src/Automata.hs
...
#+RESULTS:
#+begin_example
[1 of 1] Compiling MyMats           ( src/MyMats.hs, interpreted )
Ok, one module loaded.
a =
[1,2,3]
[4,5,6]
...
- The "AND" Operation/Function ::
  ...
  #+begin_src gnuplot :var data="./w2026/and.txt" :file ./images/my-and.png :results file :cache no
    reset
    unset key
    unset colorbox
    ...
    plot data u 1:2:3 with points ps 7 pt 7 lc palette 
#+end_src
...
- The "OR" Operation/Function ::
  ...
  #+begin_src gnuplot :var data="./w2026/or.txt" :file ./images/my-or.png :cache no
    reset
    set nokey
    set nocolorbox
    set palette model RGB defined ( 0 "red", 1 "green" )
...
- The "XOR" Operation/Function ::
  ...
  #+begin_src gnuplot :var data="./w2026/xor.txt" :file ./images/my-xor.png :cache no
    reset
    set nokey
    set nocolorbox
    set palette model RGB defined ( 0 "red", 1 "green" )
...
#+begin_src haskell :session :results output :exports both :cache no
  :set -isrc
  :load ./src/Perceptron.hs
  updWeight0 ([[0.3,0.7]],1) [[-0.6,0.8::Double]]
#+end_src

#+RESULTS:
: [1 of 2] Compiling MyMats           ( src/MyMats.hs, interpreted )
: [2 of 2] Compiling Perceptron       ( src/Perceptron.hs, interpreted )
...
* Algebraic Psychology

This section, still to be written, will present some of the mathematics of abstract algebra (not as hard as it sounds), and then show how that can be used to approach a cognitive modeling problem. The example problem we will use is a comparison of /formal concept analysis/ with [[https://en.wikipedia.org/wiki/Paul_Thagard][Paul Thagard's]] /Theory of Explanatory Coherence/ [cite:@Thagard_1989].
...
** I demand a satisfactory explanation
*** A celestial example and a Planetary Context
[[file:./images/planetCtx.png]]
...
[[file:./images/lavosier.png]]
** And the network said ..
[[file:../images/lavosierPlot.png]]
** From Lisp to R
...
** And the network said ... (ECHO, Echo, echo ...)
[[file:./images/rEcho.png]]
...
- That is, for $A \subseteq G$ and $B \subseteq M$ if we define the following (') operations thusly:
  \begin{eqnarray}
  A' &=& \{ m \in M~|~\forall g \in A~gRm\} \\
  B' &=& \{ g \in G~|~\forall m \in B~gRm\} \\
  \mbox{then, } A'' &=& A \mbox{, defines a concept.} 
  \end{eqnarray}
** A Romantic Connection
#+attr_latex: :options [Galois Connection]
#+begin_definition
@@html:<span class="definition-title">Definition: Galois Connection</span>@@
$a \leq F(b) \iff G(a) \geq b$
#+end_definition
...
** Jacob's Ladder
*** A Planetary Lattice
[[file:./images/planetLattice.png]]

*** The Context
[[file:./images/planetCtx.png]]


#+begin_src R :exports none :session *rfca*  :results silent :cache no
source("/home/britt/gitRepos/fcateq/code/echo/R/lavOxy.R")
library(hasseDiagram)
#+end_src


** From ECHO to FCA: The Lavoisier Context
#+BEGIN_SRC R :exports results :results output :session *rfca* :cache no
# Display the Lavoisier explanatory matrix
print("Lavoisier vs Phlogiston Explanatory Context:")
print(lavoisier_matrix)
#+END_SRC
...
#+end_example
Evidence E1-E8 as objects, hypotheses OH1-OH6 (Oxygen) and PH1-PH6 (Phlogiston) as attributes.
A value of 1 means the hypothesis explains the evidence.

** Sample Formal Concepts

#+BEGIN_SRC R :exports results :results output :session *rfca* :cache no
# Show first few concepts from the combined analysis
fc_lavoisier_combined$concepts$to_list()[1:5]
#+END_SRC
...
** Implications in the Lavoisier Context  
#+BEGIN_SRC R :exports results :results output :session *rfca* :cache no
# Display key implications
print("Key implications discovered:")
list(fc_lavoisier_combined$implications)[[1]][14:16]
#+END_SRC
...
** The Lavoisier Concept Lattice
#+BEGIN_SRC R :exports results :session *rfca* :results graphics file :file ./images/lvll.png :cache no
  png("./images/lvll.png", width=800, height=600, res=150)
  fc_lavoisier_combined$concepts$plot()
  dev.off()
  "lvll.png"
#+END_SRC

#+Caption: "The lattice structure reveals the hierarchical organization of explanatory relationships."
#+RESULTS[c3a61273ad102d0dcb726a9fa202d215592ad3a9]:
[[file:./images/lvll.png]]

* Reasons to Prefer FCA
** Data Processing Inequality
#+attr_latex: :options [Data Processing Inequality]
#+begin_theorem
# Inline HTML export block for the title (ignored by the LaTeX exporter)
@@html:<span class="theorem-title">Theorem: Data Processing Inequality</span>@@

For random variables $X$, $Y$, and $Z$ that form a Markov chain $X \to Y \to Z$, we have:
$$I(X;Z) \leq I(X;Y)$$
...
** Formal Contexts are Chu Spaces
#+attr_latex: :options [Chu Space]
#+begin_definition
@@html:<span class="definition-title">Definition: Chu Space</span>@@
A Chu space over a set $K$ is a triple $(A, r, X)$ where:
  - $A$ and $X$ are sets (called /points/ and /states/ respectively)
  - $r$: A \times X \to K$ is a function (the /evaluation map/)
...
** Chu Spaces are *-autonomous categories
...
** Unifying Logic, Geometry, and Computation
...
** I probably ran out of time so come talk to me
...
[cite/n:@Thagard_1989; @Wille_2005; @tull2020monoidalcategoriesformalconcept; @lopez2021chuareyou; @oberauer2019; @Davey_2002]
