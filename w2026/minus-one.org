...
using the starting weight -0.6 0.8 try a few cycles through the data by hand and then move on to writing the perceptron rule into code so that you can automate the updating step. Haskell code for some of these functions can be found [[file:src/Perceptron.hs][here]].
...
#+begin_src haskell :dir . :session :exports both :results output
  ...
  outProd = multSimpMats (binary2Bipolar hopa) $ binary2Bipolar hopb
  noSelfConnections  = zeroDiagonal outProd
  learnRate = scalarMultiply 3 noSelfConnections
...
IAMHERE. Need to write the asynchronus version for Haskell
