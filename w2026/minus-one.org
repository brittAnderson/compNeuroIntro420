...
#+BEGIN_SRC R :exports results :session *rfca* :results graphics file :file lvll.png
...
\begin{itemize}
\item \textbf{Duality}: Each Chu space has a canonical dual
\item \textbf{Internal Hom}: $[A,B] = A^* \otimes B$ 
\item \textbf{Self-duality}: The category is equivalent to its own opposite
\end{itemize}
...
#+BEGIN_EXPORT latex
\textbf{Logic:} Chu spaces model linear logic's multiplicative fragment, where $A \multimap B \equiv A^* \otimes B$
\vspace{0.3cm}
\textbf{Geometry:} Stone duality connects topological spaces with Boolean algebras via Chu construction
\vspace{0.3cm}
\textbf{Computation:} Game semantics emerges naturally—strategies are morphisms in Chu categories
\vspace{0.3cm}
\textbf{Cognition:} Formal concepts capture the duality between extension (objects) and intension (properties)
#+END_EXPORT
...
** I probably ran out of time so come talk to me\footnote{Because I actually don't know that much about most of what I am talking about. If you know more, or want to dive deeper into something we talked about today, I would love to work with you.}
