...
*** Why should a psychology student care?
    ...
    Because if this is true: that anything computable can be computed by the lambda calculus; /and/ that the mind is basically a computation, then the mind can be computed by the lambda calculas. Everything that is mental can be accomplished by this single construct of the function and application. 
...
- Index cards in three colors (suggest: blue for S, red for K, yellow for I)
- White index cards for data values (labeled a, b, c, etc.)
...
- One S person (blue card)
- One K person (red card)  
- One I person (yellow card)
- Two or three Data people (white cards with letters)

Distribute [[file:lambda-instruction-cards.org][role instruction]] cards.
