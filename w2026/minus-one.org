...
**** A digression on code
     ...
     1) Do /not/ use them to do an exercise for you. You may learn something about LLMs, but you will /not/ learn how to code that algorithm and or very much about the programming language you want to master.
	...
	- You want to know if your code is idiomatic. Ask your llm for a critique and comment on working code.
     ...
     4) Do /not/ live in the web/browser app. Get yourself an api key and access the llm from your chosen ide. Two tools that I use often are [[https://aider.chat/][aider]] and [[https://openrouter.ai/][openrouter]]. The latter will give you access to free models. Aider will allow you to call the llm from the terminal.
...
#+INCLUDE: "./code/BusyBeaver.hs" src haskell -n 

#+begin_src haskell :dir . :session :output :exports both
import BusyBeaver
--:load ./code/BusyBeaver.hs
...
*** Python Implementation :noexport:

Let's make these combinators concrete by implementing them in Python:

#+BEGIN_SRC python :session *combi* :exports results :results output
...
print("I(5) =", I(5))
print("K(3)(7) =", K(3)(7))
print("S(K)(K)(5) =", S(K)(K)(5))

# The S(K)(K) combination gives us I
print("S(K)(K)(42) behaves like I(42):", S(K)(K)(42))
#+END_SRC
...
The strength of a change in a connection is proportionate to the product of the input and outputs, i.e. \( \Delta A[i,j] = \eta f[j]g[i] \) and \( g[i] = \sum_j~A[i,j]~f[j] \) therefore, \( \vec{g} = \mathbf{Af} \).[fn:1]
...
[fn:1] Does it matter that the $\mathbf{W}$ comes first?
...
#+end_quote
  ...
  Further reflection highlights that in psychology and cognitive science we are not very good at theory creation. In fact, some argue that we have a "theory crisis" in psychology [cite:@oberauer2019].

  
** Questions
   1. Is there a theory crisis in psychology?
      You might want to read [cite:@oberauer2019] first.
   2. What should we do about it?
      You might find these citations relevant for your discussion [cite:@guest21_how_comput_model_can_force; @rooij21_theor_befor_test; @Borsboom_2021; @naur85_progr_as_theor_build; @Eronen_2020; @Koch_2025; @Oude_Maatman_2021].
   3. Is the ability to predict the tides a theory of the tides? Is it an explanation of the tides?
