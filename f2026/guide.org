...
#+begin_src haskell :dir . :session :cache no :results file graphics :file ./images/expdecay.png :exports both
...
**** Same Exercise as Homework with one of these languages /that I will assign/. I expect you will use an LLM. See the Learn Dropbox for additional instructions. 
     1) Common Lisp
     2) Go
     3) Rust
     4) Lean4
...
*** Generate your own plot of the exponential function. :exercise:
    This can be done in class, but it is not homework and is not graded. It is just something you can do to work on your skills. 
** Reprise
...
#+begin_src haskell :dir . :session :results file graphics :file ./images/derivExample.png :exports both :cache no
...
Now let's test it:
#+begin_src haskell :cache no :results output :exports both
  :load "./src/Sqrt.hs"
  import Sqrt
...
#+begin_src haskell :dir . :session :results file graphics :file ./images/spring.png :exports both :cache no
  import Spring
  :load "./src/Spring.hs"
...
#+begin_src haskell :dir . :session :results file graphics :file ./images/iandf.png :exports both :cache no
  import IandF
    :load ./src/IandF.hs
...
#+begin_src haskell :dir . :session :results file graphics :file ./images/handh.png :exports both :cache no
  import HandH
...
#+begin_src haskell :dir . :session :output :exports both :cache no
import BusyBeaver
...
#+BEGIN_SRC haskell :session *lambdademo* :exports both :results output :cache no
  :set -w
...
#+begin_src haskell :dir . :session :exports both :results output :cache no
  :load "./src/MyMats.hs"
  import MyMats                                                                   
  putStrLn $ "a = "
...
./src/MyMats.hs:37:22-25: warning: [GHC-63394] [-Wx-partial]
    In the use of ‘head’
...
./src/MyMats.hs:37:48-51: warning: [GHC-63394] [-Wx-partial]
    In the use of ‘tail’
    (imported from Prelude, but defined in GHC.Internal.List):
...
./src/MyMats.hs:54:21-24: warning: [GHC-63394] [-Wx-partial]
    In the use of ‘head’
    (imported from Prelude, but defined in GHC.Internal.List):
    "This is a partial function, it throws an error on empty lists. Use pattern matching, 'Data.List.uncons' or 'Data.Maybe.listToMaybe' instead. Consider refactoring to use "Data.List.NonEmpty"."
   |
...
#+begin_example
c =
<interactive>:1287:10-18: error: [GHC-88464]
    Variable not in scope: prettyMat :: t0 -> String

<interactive>:1287:20-23: error: [GHC-88464]
    Variable not in scope: matc
d =
<interactive>:1289:10-18: error: [GHC-88464]
    Variable not in scope: prettyMat :: t0 -> String

<interactive>:1289:20-23: error: [GHC-88464]
    Variable not in scope: matd
c * d =
<interactive>:1291:10-18: error: [GHC-88464]
    Variable not in scope: prettyMat :: a0 -> String

<interactive>:1291:22-33: error: [GHC-88464]
    Variable not in scope: multSimpMats :: t0 -> t1 -> a0

<interactive>:1291:35-38: error: [GHC-88464]
    Variable not in scope: matc

<interactive>:1291:40-43: error: [GHC-88464]
    Variable not in scope: matd
#+end_example
Classroom Exercise
#+begin_quote
...
- The "AND" Operation/Function ::
  ...
  #+begin_src gnuplot :var data="and.txt" :file ./images/my-and.png :cache no
    reset
    set nokey
    set nocolorbox
...
- The "OR" Operation/Function ::
  ...
  #+begin_src gnuplot :var data="or.txt" :file ./images/my-or.png :cache no
    reset
    set nokey
    set nocolorbox
    set palette model RGB defined ( 0 "red", 1 "green" )
...
- The "XOR" Operation/Function ::
  ...
  #+begin_src gnuplot :var data="xor.txt" :file ./images/my-xor.png :cache no
    reset
    set nokey
    set nocolorbox
    set palette model RGB defined ( 0 "red", 1 "green" )
...
#+begin_src haskell :dir . :session :results output :exports both :cache no
  updWeight0 ([[0.3,0.7]],1) [[-0.6,0.8::Double]]
#+end_src

#+RESULTS:
: <interactive>:1294:1-10: error: [GHC-88464]
:     Variable not in scope:
:       updWeight0 :: ([[a0]], b0) -> [[Double]] -> t
...
#+RESULTS:
#+begin_example
<interactive>:1298:31-38: error: [GHC-88464]
    Variable not in scope: testData :: [a0]

<interactive>:1300:16-23: error: [GHC-88464]
    Variable not in scope: oneRound :: t0 -> t1 -> [a]

<interactive>:1300:25-27: error: [GHC-88464]
    Variable not in scope: myw

<interactive>:1300:29-38: error: [GHC-88464]
    Variable not in scope: testDataFs
<interactive>:1301:1-7: error: [GHC-88464]
    Variable not in scope: checkWt :: t0 -> t1 -> t

<interactive>:1301:9-13: error: [GHC-88464]
    Variable not in scope: newwt

<interactive>:1301:15-22: error: [GHC-88464]
    Variable not in scope: testData

<interactive>:1303:17-24: error: [GHC-88464]
    Variable not in scope: oneRound :: t0 -> t1 -> [a]

<interactive>:1303:26-30: error: [GHC-88464]
    Variable not in scope: newwt

<interactive>:1303:32-41: error: [GHC-88464]
    Variable not in scope: testDataFs
<interactive>:1304:1-7: error: [GHC-88464]
    Variable not in scope: checkWt :: t0 -> t1 -> t

<interactive>:1304:9-14: error: [GHC-88464]
    Variable not in scope: newwt2

<interactive>:1304:16-23: error: [GHC-88464]
    Variable not in scope: testData
#+end_example
...
#+RESULTS:
#+begin_example
<interactive>:1307:23-29: error: [GHC-76037]
    Not in scope: type constructor or class ‘SimpMat’
<interactive>:1308:23-29: error: [GHC-76037]
    Not in scope: type constructor or class ‘SimpMat’
<interactive>:1309:12-23: error: [GHC-88464]
    Variable not in scope: multSimpMats :: t3 -> a0 -> b

<interactive>:1309:26-39: error: [GHC-88464]
    Variable not in scope: binary2Bipolar :: t2 -> t3

<interactive>:1309:42-52: error: [GHC-88464]
    Variable not in scope: rotateLList :: t1 -> t2

<interactive>:1309:54-57: error: [GHC-88464]
    Variable not in scope: hopa

<interactive>:1309:63-76: error: [GHC-88464]
    Variable not in scope: binary2Bipolar :: t0 -> a0

<interactive>:1309:78-81: error: [GHC-88464]
    Variable not in scope: hopa
<interactive>:1310:12-23: error: [GHC-88464]
    Variable not in scope: multSimpMats :: t3 -> a0 -> b

<interactive>:1310:26-39: error: [GHC-88464]
    Variable not in scope: binary2Bipolar :: t2 -> t3

<interactive>:1310:42-52: error: [GHC-88464]
    Variable not in scope: rotateLList :: t1 -> t2

<interactive>:1310:54-57: error: [GHC-88464]
    Variable not in scope: hopb

<interactive>:1310:63-76: error: [GHC-88464]
    Variable not in scope: binary2Bipolar :: t0 -> a0

<interactive>:1310:78-81: error: [GHC-88464]
    Variable not in scope: hopb
<interactive>:1311:15-26: error: [GHC-88464]
    Variable not in scope: add2SimpMats :: t0 -> t1 -> t

<interactive>:1311:28-35: error: [GHC-88464]
    Variable not in scope: outProda

<interactive>:1311:37-44: error: [GHC-88464]
    Variable not in scope: outProdb
<interactive>:1312:22-33: error: [GHC-88464]
    Variable not in scope: zeroDiagonal :: t0 -> t

<interactive>:1312:35-45: error: [GHC-88464]
    Variable not in scope: outProdBoth
<interactive>:1313:13-26: error: [GHC-88464]
    Variable not in scope: scalarMultiply :: t0 -> t1 -> t

<interactive>:1313:30-46: error: [GHC-88464]
    Variable not in scope: noSelfConnections
<interactive>:1314:10-18: error: [GHC-88464]
    Variable not in scope: prettyMat :: t0 -> String

<interactive>:1314:20-28: error: [GHC-88464]
    Variable not in scope: learnRate
#+end_example
...
#+BEGIN_SRC R :exports results :session *rfca* :results graphics file :file ./images/lvll.png :cache no
  png("lvll.png", width=800, height=600, res=150)
...
\begin{definition}[Chu Space]
A Chu space over a set $K$ is a triple $(A, r, X)$ where:
\begin{itemize}
\item $A$ and $X$ are sets (called \emph{points} and \emph{states} respectively)
\item $r: A \times X \to K$ is a function (the \emph{evaluation map})
\end{itemize}
